...
Ad ogni maglia indipendente si attribuisce una variabile, detta \textbf{corrente di maglia}, generalmente indicata con \(I_1, I_2, \dots, I_M\). Il verso di percorrenza di queste correnti è, per convenzione, arbitrario; tuttavia, per garantire uniformità e ridurre la probabilità di errori algebrici, si adotta sistematicamente il \textbf{verso orario} per tutte le maglie. Questa scelta, seppur non obbligatoria, semplifica la determinazione dei segni nelle resistenze condivise.
...
\end{circuitikz}
\caption{Circuito con tre maglie elementari. Le correnti di maglia \(I_1\), \(I_2\) e \(I_3\) sono state assegnate con verso orario. I nodi sono etichettati per riferimento.}
\label{fig:circuito_esempio}
\end{figure}

Le tre maglie indipendenti sono individuate come segue:


Si scrivono ora le equazioni di Kirchhoff delle tensioni per ciascuna maglia, percorrendole nel verso orario assegnato. Per ogni resistore si utilizza la legge di Ohm esprimendo la corrente che lo attraversa come combinazione algebrica delle correnti di maglia; per ogni generatore di tensione si introduce il suo valore con il segno appropriato a seconda che si incontri il terminale negativo o positivo durante la percorrenza.



Si osservi la struttura della matrice delle resistenze: sulla diagonale principale compaiono le somme delle resistenze di ciascuna maglia (auto-resistenze); fuori diagonale, i termini sono non nulli solo in corrispondenza di maglie che condividono una resistenza, e assumono valore negativo (con la convenzione di correnti di maglia tutte orarie). I termini noti sono le somme algebriche delle tensioni dei generatori presenti in ciascuna maglia, con il segno determinato dal verso di percorrenza.
