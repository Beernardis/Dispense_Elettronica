...
Applichiamo la procedura al circuito di Figura \ref{fig:circuito_gen_corrente}, che contiene tre maglie e due generatori di corrente. Le correnti di maglia \(I_1,I_2,I_3\) sono state scelte tutte in verso orario.
...
\begin{figure}[h]
    ...
    \end{circuitikz}
    \caption{Circuito con tre maglie e due generatori di corrente}
...
\begin{figure}[h]
    ...
    \begin{circuitikz}[scale=1.2, every node/.style={font=\small}]
        ...
        \draw (2.25, 2) to[american voltage source, l={$E_1$}] (0.75, 2);
        \draw (4, -0) to[american current source, l={$J_2$}] (4, 2);
        \draw (6, 4) to[american voltage source, l_={$E_2$}] (6, 2);
        % Nodi
        \node[circ] at (4, 4){};
        \node at (4,4) [above] {A};
        \node[circ] at (4, 0){};
    ...
    \end{circuitikz}
    \caption{Circuito con tre maglie e due generatori di corrente. Le correnti di maglia sono in senso orario.}
...
\begin{itemize}
    ...
    \item Per la \textcolor{magenta}{\textbf{terza maglia}} notiamo che il resistore $R_5$ appartiene solo alla terza maglia, mentre il resistore $R_4$ è condiviso tra la terza e la seconda maglia. Perciò l'equazione diventa:
          \begin{equation}
              V_{J_2} - R_4\cdot\left( I_3 - I_2 \right) - E_4 - R_5\cdot \left( I_3 \right) = 0
