...
Per ogni nodo diverso dal riferimento (nodi \(A\), \(C\) e \(D\)) imponiamo la somma algebrica delle correnti entranti nulla. Nelle equazioni compare anche la corrente incognita dei generatori di tensione, che indichiamo con \(I_{E_1}\) per il generatore \(E_1\) e \(I_{E_2}\) per \(E_2\).

\begin{itemize}
    \item \textcolor{red}{\textbf{Nodo A:}} 
    \begin{equation}
        J_1 - I_{R_1} - I_{R_3} + I_{E_1} = 0
        \label{eq:nodoA_vincolo}
    \end{equation}
    \item \textcolor{blue}{\textbf{Nodo B:}} 
    \begin{equation}
        -J_2 + I_{R_2} - I_{E_1} + I_{E_2} = 0
        \label{eq:nodoB_vincolo}
    \end{equation}
    \item \textcolor{magenta}{\textbf{Nodo C:}} 
    \begin{equation}
        -I_{R_2} + I_{R_3} - I_{E_2} = 0
        \label{eq:nodoC_vincolo}
    \end{equation}
\end{itemize}

Qui \(I_{R_1}, I_{R_2}, I_{R_3}\) sono le correnti nei resistori, espresse in funzione dei potenziali nodali.

\hrule
\subsection*{3. Applicare la legge di Ohm}

Esprimiamo le correnti nei resistori con la legge di Ohm, utilizzando le conduttanze \(G_i = 1/R_i\):

\begin{align}
    I_{R_1} &= G_1 (V_A - 0) = G_1 V_A, \\
    I_{R_2} &= G_2 (V_C - V_B), \\
    I_{R_3} &= G_3 (V_A - V_C).
\end{align}

Sostituendo nelle equazioni di KCL otteniamo:

\begin{align}
    \text{Nodo A:}&\quad J_1 - G_1 V_A - G_3 (V_A - V_C) + I_{E_1} = 0, \label{eq:nodoA_ohm}\\
    \text{Nodo B:}&\quad -J_2 + G_2 (V_C - V_B) - I_{E_1} + I_{E_2} = 0, \label{eq:nodoB_ohm}\\
    \text{Nodo C:}&\quad -G_2 (V_C - V_B) + G_3 (V_A - V_C) - I_{E_2} = 0. \label{eq:nodoC_ohm}
\end{align}
