\section{Problema del Generatore di Tensione}

Il metodo dei potenziali ai nodi si basa sull’applicazione della Legge di Kirchhoff delle correnti (LKC) a ciascun nodo del circuito, escluso il riferimento. Quando sono presenti generatori ideali di tensione si presenta una difficoltà: un generatore di tensione impone una differenza di potenziale nota tra due nodi, ma la corrente che lo attraversa è incognita e dipende dal resto del circuito. Non è quindi possibile esprimere direttamente la corrente nel ramo del generatore tramite la legge di Ohm.

Per risolvere il problema si introduce una procedura sistematica che:
\begin{itemize}
    \item mantiene i potenziali nodali come incognite principali;
    \item aggiunge una variabile per ogni generatore di tensione: la corrente che lo attraversa (indicata con \(I_E\));
    \item scrive le equazioni di KLC a ciascun nodo includendo queste correnti incognite;
    \item aggiunge un’equazione di vincolo per ogni generatore, che lega i potenziali nodali alla tensione imposta.
\end{itemize}

\subsection*{Esempio applicativo}

Applichiamo la procedura al circuito di Figura \ref{fig:circuito_nodi_1}, che contiene tre nodi indipendenti e due generatori di tensione. I potenziali nodali \(V_A, V_B, V_C\) sono stati definiti rispetto al nodo di riferimento \(D\) (massa).

\CircuitoPotenzialeNodi{3}  

Il procedimento è molto simile a quello spiegato nel capitolo precedente, con l’aggiunta del punto 3.1:

\begin{enumerate}
    \item[1.] \textcolor{gray}{\textbf{Identificare i nodi e scegliere il nodo di riferimento:} Si individua il nodo di riferimento (massa) e si assegnano potenziali incogniti agli \(N-1\) nodi rimanenti.}
    \item[2.] \textcolor{gray}{\textbf{Scrivere le equazioni nodali (LKC):} Per ogni nodo diverso dal riferimento si impone la somma algebrica delle correnti nulla.}
    \item[3.] \textcolor{gray}{\textbf{Applicare Ohm:} Utilizzare la legge di Ohm per esprimere le correnti nei resistori in funzione dei potenziali nodali.}
    \item[3.1.] \textbf{Equazioni di vincolo dei generatori di tensione:} Per ogni generatore di tensione si aggiunge una equazione che lega la differenza di potenziale tra i due nodi a cui è collegato al valore della tensione impressa; inoltre si introduce la corrente incognita del generatore nelle equazioni di KCL dei nodi interessati.
    \item[4.] \textcolor{gray}{\textbf{BONUS: Matrici!} Organizzare le equazioni in forma matriciale, identificando la matrice delle conduttanze e il vettore dei termini noti.}
\end{enumerate}

\subsection*{1. Identificare i nodi e scegliere il nodo di riferimento}

Nel circuito di Figura \ref{fig:potenziale_3} sono presenti \(N=4\) nodi. Scegliamo come riferimento il nodo \(B\) (massa). Assegniamo quindi i potenziali incogniti \(V_A\), \(V_C\) e \(V_D\) ai nodi rimanenti.

\CircuitoPotenzialeNodi{4}

\hrule
\subsection*{2. Scrivere le equazioni nodali (LKC)}

Per ogni nodo diverso dal riferimento (nodi \(A\), \(C\) e \(D\)) imponiamo la somma algebrica delle correnti entranti nulla. Nelle equazioni compare anche la corrente incognita dei generatori di tensione, che indichiamo con \(I_{E_1}\) per il generatore \(E_1\) e \(I_{E_2}\) per \(E_2\). Consideriamo il verso della corrente nei rami con i generatori di tensione uscente dal morsetto positivo degli stessi.


\paragraph{\textcolor{red}{\textbf{Nodo $A$:}}}
\begin{minipage}[t]{0.6\textwidth}
\vspace{0pt}
Dal nodo $A$ partono tre rami, quindi ci saranno 3 fattori da considerare nell'equazione: la corrente imposta dai due generatori di corrente e la corrente sul resistore. Prendiamo come verso positivo le correnti entranti nel nodo:

\begin{equation}
        J_1 - I_{R_4} + I_{E_2} = 0
        \label{eq:nodoA_vincolo}
\end{equation}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
\vspace{0pt}
\centering
\ZoomNodo{A_1}
\end{minipage}

\vspace{0.5cm}
\hrule
\vspace{0.5cm}

\paragraph{\textcolor{blue}{\textbf{Nodo $C$:}}}
\begin{minipage}[t]{0.6\textwidth}
\vspace{0pt}
Dal nodo $C$ partono tre rami, quindi ci saranno 3 fattori da considerare nell'equazione: la corrente imposta dai due generatori di corrente e la corrente sul resistore. Prendiamo come verso positivo le correnti entranti nel nodo:

\begin{equation}
        -J_1 - I_{R_1} - I_{E_1} = 0
        \label{eq:nodoC_vincolo}
\end{equation}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
\vspace{0pt}
\centering
\ZoomNodo{C_1}
\end{minipage}

\vspace{0.5cm}
\hrule
\vspace{0.5cm}

\paragraph{\textcolor{magenta}{\textbf{Nodo $D$:}}}
\begin{minipage}[t]{0.6\textwidth}
\vspace{0pt}
Dal nodo $D$ partono tre rami, quindi ci saranno 3 fattori da considerare nell'equazione: la corrente imposta dai due generatori di corrente e la corrente sul resistore. Prendiamo come verso positivo le correnti entranti nel nodo:

\begin{equation}
        J_2 + I_{E_1} - I_{R_4} = 0
        \label{eq:nodoD_vincolo}
\end{equation}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
\vspace{0pt}
\centering
\ZoomNodo{D_1}
\end{minipage}


\vspace*{0.3cm}
\hrule

\subsection*{3. Applicare la legge di Ohm}
Per ogni equazione bisogna definire ogni corrente come rapporto tra resistenza complessiva sul ramo e tensione tra i nodi. Inoltre riscriviamo sempre l'equazione utilizzando il concetto di \textbf{conduttanza}.

\paragraph{Nodo A}
Oltre alla corrente incognita sul ramo di $R_4$ c'è anche la corrente incognita del generatore $E_2$. Perciò l'equazione diventa:
\begin{equation}
J_1 - \frac{V_A - V_D}{R_4} + I_{E_2}= 0 \qquad \Rightarrow \qquad J_1 - G_4\left(V_A - V_D\right) + I_{E_2}= 0
\label{eq:nodoA_Ohm_E}
\end{equation}

\paragraph{Nodo C}
Oltre alla corrente incognita sul ramo di $R_1$ c'è anche la corrente incognita del generatore $E_1$. Perciò l'equazione diventa:
\begin{equation}
- J_1 - \frac{V_C - V_B}{R_1} - I_{E_1} = 0 \qquad \Rightarrow \qquad - J_1 - G_1\left(V_C\right) - I_{E_1}= 0
\label{eq:nodoC_Ohm_E}
\end{equation}


\paragraph{Nodo D}
Oltre alla corrente incognita sul ramo di $R_4$ c'è anche la corrente incognita del generatore $E_2$. Perciò l'equazione diventa:
\begin{equation}
J_2 - \frac{V_D - V_A}{R_4} + I_{E_1}= 0 \qquad \Rightarrow \qquad J_2 - G_4\left(V_D-V_A\right) + I_{E_1} = 0
\label{eq:nodoD_Ohm_E}
\end{equation}









\hrule
\subsection*{3.1. Equazioni di vincolo dei generatori di tensione}

Ogni generatore di tensione impone una relazione tra i potenziali dei nodi a cui è collegato.

\begin{itemize}
    \item Generatore \(E_1\): è collegato tra il nodo \(D\) e il nodo \(C\). La tensione impressa, con la polarità indicata in figura, è:
          \begin{equation}
              V_D - V_C = E_1 + R_3I_{A \rightarrow B} = E_1 - R_3I_{E_1} \qquad \Rightarrow \qquad V_D - V_C + R_3I_{E_1} = E_1
              \label{eq:vincoloE1}
          \end{equation}
    Ha molto senso riscrivere l'equazione in maniera da usare solo conduttanze, quindi possiamo dividere per $R_3$, o moltiplicare per $G_3$ visto che è la stessa operazione, entrambi i membri per ottenere:
    \begin{equation}
        G_3V_D - G_3V_C + I_{E_1} = G_3E_1
        \label{eq:vincoloE1Conduttanze}
    \end{equation}
    \item Generatore \(E_2\): è collegato tra il nodo \(A\) e il nodo \(B\). La tensione impressa è:
          \begin{equation}
              V_A - V_B = E_2 + R_5I_{A \rightarrow B} = E_2 - R_5I_{E_2} \qquad \Rightarrow \qquad V_A - V_B + R_5I_{E_2} = E_2
              \label{eq:vincoloE2}
          \end{equation}
    Ci ricordiamo che il nodo $B$ è connesso a massa, quindi il suo potenziale è nullo. Ha poi molto senso riscrivere l'equazione in maniera da usare solo conduttanze, quindi possiamo dividere per $R_5$, o moltiplicare per $G_5$ visto che è la stessa operazione, entrambi i membri per ottenere:
    \begin{equation}
        G_5V_A + I_{E_2} = G_3E_2
        \label{eq:vincoloE2Conduttanze}
    \end{equation}
\end{itemize}


Queste due equazioni, insieme alle tre equazioni di KCL, formano un sistema di 5 equazioni nelle 5 incognite \(V_A, V_B, V_C, I_{E_1}, I_{E_2}\).


Per una migliore comprensione, possiamo mettere a sistema le equazioni trovate e riorganizziamo i termini per esplicitare le varie incognite.
Quindi il nostro sistema si potrà riscrivere in questa maniera:

\begin{equation}
    \left\{
\begin{aligned}
G_4&\textcolor{red}{V_A} & +0&\textcolor{blue}{V_C} & - G_4&\textcolor{magenta}{V_D} & +0 &\textcolor{brown}{I_{E_1}} & - 1&\textcolor{orange}{I_{E_2}} & = & J_1 \\
0&\textcolor{red}{V_A} & + G_1&\textcolor{blue}{V_C} & +0 &\textcolor{magenta}{V_D} & + 1 &\textcolor{brown}{I_{E_1}} & +0&\textcolor{orange}{I_{E_2}} & = & - J_1 \\
-G_4&\textcolor{red}{V_A} & +0&\textcolor{blue}{V_C} & + G_4&\textcolor{magenta}{V_D} & +0& \textcolor{brown}{I_{E_1}} & - 1&\textcolor{orange}{I_{E_2}} & = & J_2 \\
0&\textcolor{red}{V_A} & - G_3&\textcolor{blue}{V_C} & + G_3&\textcolor{magenta}{V_D} & + 1 &\textcolor{brown}{I_{E_1}} & +0&\textcolor{orange}{I_{E_2}} & = & G_3E_1 \\
G_5&\textcolor{red}{V_A} & +0&\textcolor{blue}{V_C} & +0 &\textcolor{magenta}{V_D} & +0 &\textcolor{brown}{I_{E_1}} & + 1&\textcolor{orange}{I_{E_2}} & = & G_5E_2
\end{aligned}
\right.
\end{equation}



\hrule
\subsection*{4. BONUS: Matrici!}

Anche in questo caso la rappresentazione matriciale semplifica la gestione del sistema.

\subsubsection*{Matrice dei coefficienti}

La matrice dei coefficienti \(\mathbf{A}\) può essere suddivisa in blocchi che riflettono la struttura del problema:

\begin{equation}
    \mathbf{A} = 
    \begin{tikzpicture}[baseline=(m.center)]
        \node (m) [matrix of math nodes,left delimiter={[},right delimiter={]}]
            {
                +G_4 & 0 & -G_4 & 0 & -1 \\
                0 & +G_1 & 0 & +1 & 0 \\
                -G_4 & 0 & +G_4 & 0 & -1 \\
                0 & -G_3 & +G_3 & +1 & 0 \\
                +G_5 & 0 & 0 & 0 & +1\\
            };

        \draw[red, thick, rounded corners]
        ([xshift=-1mm,yshift=0 mm]m-1-1.north west) rectangle
        ([xshift=0mm,yshift=0.3mm]m-3-3.south east);

        \draw[green, thick, rounded corners]
        ([xshift=0.7mm,yshift=-.3 mm]m-4-4.north west) rectangle
        ([xshift=-0.7mm,yshift=0.3mm]m-5-5.south east);

        \draw[blue, thick, rounded corners]
        ([xshift=-4mm,yshift=-0.3 mm]m-4-1.north west) rectangle
        ([xshift=3mm,yshift=0mm]m-5-3.south east);

        \draw[brown, thick, rounded corners]
        ([xshift=-1mm,yshift=0 mm]m-1-4.north west) rectangle
        ([xshift=-0.8mm,yshift=0mm]m-3-5.south east);
    \end{tikzpicture}
\end{equation}

dove:
\begin{itemize}
    \item \textbf{Sottomatrice rossa:} (3$\times$3)è la matrice delle conduttanze nodali, costruita con la regola classica: \(G_{ii}\) somma delle conduttanze incidenti al nodo \(i\), \(G_{ij}\) con segno negativo per i rami resistivi tra nodi \(i\) e \(j\).
    \item \textbf{Sottomatrice marrone:} (3$\times$2) descrive come le correnti dei generatori di tensione compaiono nelle equazioni di LKC: \(C_{ik} = +1\) se la corrente \(I_{E_k}\) entra nel nodo \(i\), \(-1\) se ne esce, \(0\) altrimenti.
    \item \textbf{Sottomatrice blu:} (2$\times$3) è la matrice dei vincoli di tensione: ogni riga rappresenta un generatore di tensione e contiene \(\pm G_{ij}\) per i nodi \(i\) e \(j\) collegati dal generatore, con il segno che riflette la polarità della tensione. Ad esempio, \(E_1\) è collegato tra \(D\) e \(C\) col morsetto positivo su \(D\): per cui i segni delle conduttanze sono positive per l'incognita \(V_D\) e negative per l'incoglinta \(V_C\). La conduttanza usata per moltiplicare le incognite è la conduttanza presente tra i nodi presi in considerazione. 
    \item \textbf{Sottomatrice verde:} (2$\times$2) è la matrice che tiene conto delle cadute di potenziale ai capi dei resistori in serie ai generatori di tensione. Se la corrente del generatore \(E_k\) attraversa un resistore \(R\) con conduttanza \(G\), allora \(I_{kk} = 1\) e \(E_{k}\) include un termine \(G I_{E_k}\) nella sua equazione di vincolo.
\end{itemize}

\subsubsection*{Vettore incognita}

Il vettore incognita contiene tutti i potenziali nodali e tutte le correnti dei generatori di tensione:

\begin{equation}
    \mathbf{x} = 
    \begin{bmatrix}
        V_A \\ V_C \\ V_D \\ I_{E_1} \\ I_{E_2}
    \end{bmatrix}
\end{equation}

\subsubsection*{Vettore dei termini noti}

Il vettore \(\mathbf{b}\) raccoglie i contributi dei generatori indipendenti di corrente e delle tensioni impresse:

\begin{equation}
    \mathbf{b} = 
    \begin{bmatrix}
    J_1 \\ - J_1 \\ J_2 \\ G_3E_1 \\ G_5E_2
    \end{bmatrix}
\end{equation}

Si noti come i primi tre elementi del vettore dei termini noti siano le correnti imposte dai generatori di corrente che incidono sui nodi con segno positivo se entranti nel nodo, con segno negativo se uscenti. Mentre gli ultimi due elementi siano i prodotti tra le conduttanze dei resistori in serie ai generatori di tensione e le tensioni impresse dagli stessi generatori, sempre con segno positivo.

In forma compatta:

\begin{equation}
    \mathbf{A} \, \mathbf{x} = \mathbf{b}.
\end{equation}

La struttura a blocchi evidenzia come la presenza dei generatori di tensione estenda il sistema originale delle conduttanze con nuove variabili e nuove equazioni. Questo approccio permette di applicare metodi risolutivi standard (eliminazione di Gauss, decomposizione LU).