...
\begin{figure}[h]
    ...
    \begin{circuitikz}[scale=1.2, every node/.style={font=\small}]
        ...
        % Ground (massa) sul nodo B (4,0)
        \node[ground] at (4,0) {};
        
        % Generatori secondo il caso
        \ifcase#1
            % caso 0 non usato
        \or % #1 = 1: tutti generatori di corrente (sostituiti ai gen di tensione)
            \draw (0.75,4) to[american current source, l={$J_1$}] (4,4);
        ...
        \or % #1 = 2: generatori misti (due di corrente e due di tensione)
            \draw (0.75,4) to[american current source, l={$J_1$}] (4,4);
            \draw (2.25,2) to[american voltage source, l={$E_1$}] (0.75,2);
            \draw (4,0) to[american current source, l={$J_2$}] (4,2);
            \draw (6,4) to[american voltage source, l_={$E_4$}] (6,2);
    ...
    \caption{%
        \ifcase#1
        \or Circuito per il metodo dei potenziali ai nodi (solo generatori di corrente)
        \or Circuito per il metodo dei potenziali ai nodi (generatori di corrente e tensione)
        \fi
    }
