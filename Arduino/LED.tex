...
\begin{figure}[h]
    ...
    \begin{minipage}{0.45\textwidth}  % Larghezza della minipagina per l'immagine
        \centering
        \includegraphics[width=0.71\textwidth]{img/Circuito_Led_Arduino.png}
...
\begin{tabular}{>{\bfseries}l c c c c}
    ...
    Magenta  & ON  & OFF & ON  \\
    Bianco   & ON  & ON  & ON  \\
    Nero (spento) & OFF & OFF & OFF \\
    \bottomrule
\end{tabular}
\end{center}

Variando l'intensità di ciascun canale tramite PWM si ottengono infinite sfumature. Ad esempio, un rosso intenso si ottiene con R=255, G=0, B=0; un arancione si può ottenere con R=255, G=80, B=0.

\subsection*{Schema di collegamento}

Colleghiamo un LED RGB a catodo comune ai pin digitali di Arduino. Utilizziamo tre resistenze di limitazione, una per ogni colore. Il calcolo della resistenza è identico a quello visto per il LED singolo, usando la $V_f$ di ogni colore (tipicamente: rosso $\SI{2.0}{\volt}$, verde $\SI{2.2}{\volt}$, blu $\SI{3.2}{\volt}$). Per semplicità, con alimentazione a \SI{5}{\volt} e una corrente di circa \SI{10}{\milli\ampere}, si possono usare resistenze da \SI{220}{\ohm} per tutti e tre i canali.

\begin{figure}[h]
    \centering
    \begin{circuitikz}[scale=1.2, american]
        \draw (0,0) node[ground]{} to[short] (0,1) node[below]{GND};
        \draw (0,1) to[led, l=Rosso, color=red] (2,1) to[R, l=220$\Omega$] (2,3) to[short] (3,3) node[above]{Pin 9};
        \draw (0,1) to[led, l=Verde, color=green] (0,3) to[R, l=220$\Omega$] (0,5) to[short] (1,5) node[above]{Pin 10};
        \draw (0,1) to[led, l=Blu, color=blue] (-2,1) to[R, l=220$\Omega$] (-2,3) to[short] (-3,3) node[above]{Pin 11};
    \end{circuitikz}
    \caption{Collegamento di un LED RGB a catodo comune con Arduino. Il terminale comune va a GND.}
    \label{fig:led_rgb_schema}
\end{figure}
...
\begin{itemize}
    \item \textbf{Riga 2} - \texttt{pinMode(6, OUTPUT);}: eseguita una sola volta all'avvio, configura il pin 13 come uscita digitale.
    \item \textbf{Riga 6} - \texttt{digitalWrite(6, HIGH);}: porta il pin 13 a \SI{5}{\volt}. La corrente scorre attraverso la resistenza e il LED, che si accende.
    \item \textbf{Riga 7 e Riga 9} - \texttt{delay(1000);}: il programma si blocca per 1000 ms. Durante questo tempo il LED rimane nello stato in cui si trova.
    \item \textbf{Riga 8} - \texttt{digitalWrite(6, LOW);}: porta il pin 13 a \SI{0}{\volt}. Nessuna differenza di tensione, nessuna corrente, LED spento.
\end{itemize}
...
void loop() {
    analogWrite(6, 51);     // Accende il LED con duty cycle 20 %
    delay(1000);            // Aspetta 1000 millisecondi 
    analogWrite(6, 0);      // Spegne il LED con duty cycle 0 %
    delay(1000);            // Aspetta 1000 millisecondi 
    analogWrite(6, 204);    // Accende il LED con duty cycle 80 %
    delay(1000);            // Aspetta 1000 millisecondi 
    analogWrite(6, 0);      // Spegne il LED con duty cycle 0 %
    delay(1000);            // Aspetta 1000 millisecondi 
}
